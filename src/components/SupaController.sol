...
contract SupaController {
    ...
    mapping(bytes32 claimAttestationID => address TokenClaimed) public claimedTokens;
     ...
     */
    function registerSchema(string calldata schema, string memory name, string memory symbol)
        external
        returns (bytes32 schemaUID, address token)
    {
    ...
    function claim(bytes32 claimAttestationID, address receiver) external {
        ...
        token.mint(receiver, abi.decode(parseMsgData(attestation.data), (uint256)));
    }
    ...
    function reward(bytes32 rewardAttestationID, address receiver) external {
        Attestation memory attestation = eas.getAttestation(rewardAttestationID);
        ...
        token.mint(receiver, rewardAmount);
        token.approve(address(supaShrine), rewardAmount);
        supaShrine.reward(claimedToken, address(token), rewardAmount);
    }
